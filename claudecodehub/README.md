...
**ClaudeCodeHub** is a curated directory of open-source **hooks, plugins, RAG tools, subagents, workflows, and slash commands for [Claude Code](https://claude.ai/code)** — a browsable gallery where you can discover community-built hooks (security, formatting, notifications, logging, testing, automation, workflow), SDLC-focused plugins (planning, coding, code review, testing, CI/CD, deployment, monitoring, documentation), the most popular RAG/retrieval tooling for grounding Claude Code in your code and docs (code-retrieval MCP servers, vector databases, RAG frameworks, ingestion, embeddings/reranking, evaluation, memory), a top-100 curated list of subagents (architecture, backend Python, security/compliance, AWS serverless, testing/QA, code review, data persistence, documentation, DevOps/CI-CD), a curated list of end-to-end Claude Code workflows/orchestration frameworks, and a curated list of individual Claude Code slash commands (SDLC productivity, architecture & design patterns, distributed architecture, information security & compliance, AWS serverless/cloud deployment, banking & payment systems) — the Agentes, Workflows, and Commands tabs are all specifically curated for fintech/banking teams building PCI DSS-, OWASP-, and ISO 27001-conscious software on a DDD/clean/hexagonal-architecture, 100% Python, 100% AWS-serverless stack — each linking straight to its source (GitHub repo, or vendor site for the couple of API-only entries).
...
- **Gallery view** — hooks, plugins, RAG tools, agents, workflows, and commands rendered as cards in a responsive grid (1 column on mobile, up to 3 on desktop).
- **Hooks / Plugins / RAG / Agentes / Workflows / Commands toggle** — a tab switcher on the home page filters the gallery between the six catalogs.
- **Category badges** — hooks are tagged with one of `security`, `formatting`, `notifications`, `logging`, `testing`, `automation`, `workflow`; plugins are tagged by SDLC phase: `planning`, `coding`, `code-review`, `testing`, `ci-cd`, `deployment`, `monitoring`, `documentation`; RAG entries are tagged by function: `code-retrieval`, `vector-db`, `framework`, `ingestion`, `embeddings-rerank`, `evaluation`, `memory`; agents (the Agentes tab) are tagged by SDLC role: `architecture`, `backend-python`, `security-compliance`, `aws-serverless`, `testing-qa`, `code-review`, `data-persistence`, `documentation`, `devops-cicd`; workflows (the Workflows tab) are tagged by orchestration focus: `sdlc-orchestration`, `architecture-design`, `distributed-architecture`, `security-compliance`, `cloud-infrastructure`, `fintech-payments`; commands (the Commands tab) are tagged by focus: `sdlc-productivity`, `architecture-design`, `distributed-architecture`, `security-compliance`, `cloud-deployment`, `fintech-payments`.
- **Stack tag filter** — an optional, multi-select filter (chips below the Hooks/Plugins/RAG/Agentes/Workflows/Commands toggle) for narrowing the gallery to entries relevant to a specific tech stack: `python`, `aws`, `aws-lambda`, `aws-api-gateway`, `aws-dynamodb`, `aws-s3`, `clean-architecture`, `hexagonal-architecture`, `distributed-systems`, `resilience`, `observability`, `banking`, `payments`, `ddd`, `design-patterns`, `enterprise-integration-patterns`, `owasp`, `pci-dss`, `iso27001`, `best-practices`, `ai-assisted-sdlc`. Entries without a matching tag just don't appear when a filter is active — `stackTags` is optional per entry.
...
- **Workflows tab** — a curated list of end-to-end Claude Code workflow and multi-agent orchestration frameworks (as opposed to single hooks or subagents), sourced from official Anthropic docs and popular repos with real GitHub traction (e.g. `ruvnet/claude-flow`, `rohitg00/awesome-claude-code-toolkit`, `CloudAI-X/claude-workflow-v2`, `aws-samples/sample-claude-code-agent-team`, `Kaademos/secure-sdlc-agents`, `wshobson/agents`), curated for the same fintech/banking, DDD/distributed-architecture, AWS-serverless, and information-security audience as the Agentes tab.
- **Commands tab** — a curated list of individual Claude Code slash commands (single Markdown prompt files under `.claude/commands/`, now unified with Skills), linked to their specific source file rather than the whole repo — sourced from Anthropic's official commands docs and popular repos (`wshobson/commands`, `dykyi-roman/awesome-claude-code`, `rohitg00/awesome-claude-code-toolkit`, `afiqiqmal/claude-security-audit`), curated for the same fintech/banking, DDD/distributed-architecture, AWS-serverless, and information-security audience as Agentes/Workflows.
...
A scheduled GitHub Action searches the GitHub Search API every week for new hooks, plugins, RAG tools, agents, workflows, and commands worth adding to the catalog — trusted-org and vendor repos, and community repos tagged with Claude Code-specific topics — and opens a PR with what it finds. It never touches `data/catalog.ts` or the live site directly; it only appends to a separate curation queue, `data/candidates.json`, which the gallery doesn't render.
...
- **Topic search** — GitHub topics like `claude-code`, `claude-code-hook`, `claude-code-plugin`, `claude-code-agent`, `claude-subagent`, `claude-skill`, `claude-code-workflow`, `claude-code-workflows`, `claude-code-command`, `claude-code-commands`, etc. (including agent/subagent-specific topics so new subagent collections surface for the Agentes tab, workflow-specific topics so new orchestration frameworks surface for the Workflows tab, and command-specific topics so new slash-command collections surface for the Commands tab), combined with the same `"claude code"` phrase requirement, sorted by stars. Requires ≥50 stars and a push within the last 90 days, to filter out abandoned or barely-touched repos.
...
- **Good fit** — move it into the matching array in `data/catalog.ts` (`hooks`, `plugins`, `rag`, `agents`, `workflows`, or `commands`), filling in `type`, `category`, and optional `stackTags`/`official`, then delete it from `candidates.json`. For repos that bundle multiple subagents (e.g. a `.claude/agents/` or `plugins/*/agents/` directory), consider adding each individually relevant subagent as its own `agents` entry rather than one entry for the whole repo — see [Adding a new hook, plugin, RAG tool, agent, workflow, or command](#adding-a-new-hook-plugin-rag-tool-agent-workflow-or-command-to-the-gallery) above. Reserve the `workflows` array for end-to-end orchestration frameworks, and the `commands` array for individual slash commands linked to their specific file — not single hooks or whole subagent collections.
...
## Adding a new hook, plugin, RAG tool, agent, workflow, or command to the gallery

The catalog is a plain TypeScript array — no build step or database migration needed. To add an entry, add a new object to the `hooks`, `plugins`, `rag`, `agents`, `workflows`, or `commands` array in [`data/catalog.ts`](data/catalog.ts) matching the `CatalogItem` interface:
...
// Workflow
{
  name: "My Workflow",
  type: "workflow",
  category: "sdlc-orchestration", // sdlc-orchestration | architecture-design | distributed-architecture | security-compliance | cloud-infrastructure | fintech-payments
  description: "A short description of the end-to-end workflow/orchestration pattern it implements.",
  repoUrl: "https://github.com/owner/repo",
  stackTags: ["ddd", "distributed-systems"], // optional — see the stack tag list above
  official: false,
  stars: 42,
}
...
// Command
{
  name: "/my-command",
  type: "command",
  category: "security-compliance", // sdlc-productivity | architecture-design | distributed-architecture | security-compliance | cloud-deployment | fintech-payments
  description: "A short description of what the slash command does.",
  repoUrl: "https://github.com/owner/repo/blob/main/path/to/command.md",
  stackTags: ["owasp"], // optional — see the stack tag list above
  official: false,
  stars: 42,
}
...
Save the file and the new card will appear in the gallery (under the matching Hooks/Plugins/RAG/Agentes/Workflows/Commands tab) automatically the next time the page renders. For the RAG tab specifically, keep the bar high — it's meant to surface the most popular, actively maintained tools in each category, not every RAG-adjacent repo on GitHub. The Agentes, Workflows, and Commands tabs are curated for a fintech/banking, DDD/hexagonal-architecture, 100% Python, 100% AWS-serverless audience — favor real subagents/workflows/commands (from official Anthropic docs/plugins or popular community collections) whose role maps cleanly onto architecture, security/compliance (PCI DSS, OWASP, ISO 27001), AWS serverless infra, distributed/microservices architecture, banking/payment systems, or the rest of a Python SDLC, and tag them with the relevant `stackTags` even when the source repo itself is generic rather than fintech-specific. The Workflows tab specifically favors end-to-end orchestration frameworks (multi-agent pipelines, full-SDLC plugin bundles) over single hooks or individual subagents. The Commands tab favors individual, atomic slash commands (one Markdown prompt file, linked to that specific file rather than the whole repo) over multi-step orchestration — those belong in Workflows.
