...
- Twitter integration has a mock implementation (`scrape_user_tweets_mock`) for testing, and `ice_breaker.py` always calls it rather than the real `scrape_user_tweets`
- All 5 `TWITTER_*` env vars are still required at startup even though only mocked tweet data is used: `third_parties/twitter.py` constructs a `tweepy.Client` at module import time via `os.environ[...]`, so the app fails to start without them
